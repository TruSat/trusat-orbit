...
cpdef unit_vector_raw(double [:] v):
    ...
    cdef double magv
    cdef int i
    cdef array u = array('d', [0.0, 0.0, 0.0])
...
cpdef smult(double a, double [:] v):
    """ av = a * v """
    cdef int i
    cdef array av = array('d', [0.0, 0.0, 0.0])

    for i in range(3):
        av[i] = a * v[i]
    return av

cpdef vadd(double [:] v1, double [:] v2):
    """ s = v1 + v2 """
    cdef array s = array('d', [0.0, 0.0, 0.0])
    cdef int i
...
cpdef vmadd(double [:] v1, double [:] v2, double a):
...
cpdef cross(double [:] v1, double [:] v2):
    """ b = v1 x v2  """
    cdef array b = array('d', [0.0, 0.0, 0.0])
...
cpdef proj(double [:] v2, double [:] v1):
    """ unit vector projection of v1 onto v2 """
    cdef double b
    cdef array temp = array('d', [0.0, 0.0, 0.0])
    cdef array vp   = array('d', [0.0, 0.0, 0.0])

    b = dot(v2, v1)/dot(v2, v2)
    temp = smult(b, v2)
    vp = unit_vector_raw(temp)
    return vp
